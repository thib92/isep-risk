	...
	protected void updateAndRender(int delta) throws SlickException {
		...
		if (!paused) {
			...
			if (storedDelta >= minimumLogicInterval) {
				try {
					...
					Log.error(e);
					throw new SlickException("SlickGame.SlickGame.update() failure - check the game code.");
				}
			}
		} else {
			game.update(this, 0);
		...
		if (hasFocus() || getAlwaysRender()) {
			...
			try {
				...
				Log.error(e);
				throw new SlickException("SlickGame.SlickGame.render() failure - check the game code.");
			}
			graphics.resetTransform();
